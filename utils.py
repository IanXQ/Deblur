...
def write_list(train=True):
    gopro_dataset = r"/home/jihuazhu/xuqipeng/SRN-Pytorch/data/GOPRO_LARGE"
    gopro_dataset_train = gopro_dataset + "/train/"
    gopro_dataset_test = gopro_dataset + "/test/"

    dir_list = os.listdir(gopro_dataset_train) if train else os.listdir(gopro_dataset_test)
    image_blur = []
    image_sharp = []
    for dn in dir_list:
        dir_path_blur = (gopro_dataset_train if train else gopro_dataset_test) + dn + "/blur/"
        dir_path_sharp = (gopro_dataset_train if train else gopro_dataset_test) + dn + "/sharp/"
        image_names = os.listdir(dir_path_blur)
        image_blur.extend([dir_path_blur + name for name in image_names])
        image_names = os.listdir(dir_path_sharp)
        image_sharp.extend([dir_path_sharp + name for name in image_names])
    with open("train.txt" if train == True else "test.txt", "w") as f:
        train_list = []
        for i in range(len(image_blur)):
            train_list.append(image_blur[i] + " " + image_sharp[i] + '\n')
